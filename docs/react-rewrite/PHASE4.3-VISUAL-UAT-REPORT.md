...
- `e6f88c2 fix: keep reverse armed on upward scroll`
...
- `ARMED` cancellation is direction-aware: upward input cancels forward intent, while downward input cancels reverse intent. Continuing upward during a reverse arm no longer self-cancels.
...
- `scripts/phase43-visual-uat.mjs` validates full forward traversal to `contact`, reverse armed same-direction continuation, forces the `pattern-bloom` reverse boundary to `scrollY=0`, and then validates reverse traversal back to `hero`.
...
- `validate:phase42-bundle`: main gzip `78899`, GSAP gzip `27017`.
...
```json
{
  ...
  "reverseArmedContinuation": {
    "phase": "IDLE",
    "activeScene": "pattern-bloom",
    "activeSegment": null,
    "lastEvent": "RELEASE_COMPLETE",
    "direction": "reverse",
    "scrollLocked": false
  },
...
- Reverse armed state is not cancelled by continued same-direction upward input.
