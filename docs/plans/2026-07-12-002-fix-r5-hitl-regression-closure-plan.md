...
Candidate-v3 through candidate-v6 remain immutable and unqualified for their recorded dirty-tree, browser, release-oracle, and CI identity failures. Candidate-v7 is also immutable and unqualified at source `d0daed5adb83fbeff7c61e0e351673fc4dea4ff5` (annotated tag object `2f8049a3e83b393de0056287cdc16e8d79986ddf`): review found that reverse Figure2 depth Ink had no active person surfaces and that empty Ubuntu RSS samples could report `0` and pass. Commit `a0e4185` keeps the paused reverse terminal pair visible through depth Ink; `f284e48` adds platform-aware process discovery, requires browser/GPU/renderer samples, requires two distinct passing runs, and routes candidate-v8 qualification to `macos-14`. The release runner now also rejects any nonzero profiler exit before reading its report. The repeated nonbrowser gate passes 84 files / 573 tests plus lint, typecheck, build, release/static checks, and unchanged budgets. Candidate-v8 is the only allowed successor; its exact identity, two-run RSS/finalization, rollback, remote workflow, and final E2E remain qualification gates.
