...
状态：candidate-v5 已通过 exact identity-bound RSS/finalization、HTTP smokes、同端口 rollback 与 44/44 默认 E2E，但 release matrix 仅通过 49/54 applicable cases（另有 42 个 project-declared skips），因此保持不可变且 unqualified。五个失败均来自旧媒体 oracle：TTG 四项目未承认 terminal still，AOD 一项未承认端点重建也失败后留在当前静态 hold。`5785ce5` 已对齐既有合同。下一步只允许冻结新的 `react-refactor-r5-parity-repair-candidate-v6`，重复所有 exact gates，并最后运行两套 E2E 后交给 HITL。当前仍不允许合并/部署 `main`、创建 cutover tag 或开始 R6。
