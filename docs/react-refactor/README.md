...
状态：R5 HITL regression closure 已完成，candidate-v2 的 pre-freeze 自动化复验已通过并获准冻结。接下来只创建不可变候选、完成 exact-tag identity/smoke、同端口 rollback 和最终 E2E，然后停止等待用户 HITL。旧 HITL 拒绝仍有效；当前不允许合并/部署 `main`、创建 cutover tag 或开始 R6。
...
- pre-freeze 自动验收已通过；唯一允许创建的新候选 tag 是 `react-refactor-r5-parity-repair-candidate-v2`。tag object、identity-bound manifest、rollback 与最终 exact-tag E2E 作为冻结后的外部交付记录。
