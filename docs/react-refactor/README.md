...
状态：R5 HITL regression closure 已完成 Units 0–7，实现进入 candidate-v2 最终复验。旧 HITL 拒绝仍有效；只有完整自动化、exact-tag smoke 与同端口 rollback 全部通过后才冻结新候选并停止等待 HITL。当前不允许合并/部署 `main`、创建 cutover tag 或开始 R6。
...
- 最终自动验收全部通过后，唯一允许创建的新候选 tag 是 `react-refactor-r5-parity-repair-candidate-v2`。
