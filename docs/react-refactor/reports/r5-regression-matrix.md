...
Status: **pre-visual, untagged, unqualified.** Candidate-v2 through candidate-v8 are immutable historical/unqualified records. The current R5 `assets/` and production runtime implementation under `app/src/` excluding tests are identical to Batch C terminal `b62ba647cbf5402299cd0a5eef46fff152c48524`; release-control workflow/tests/tooling are outside that identity boundary. Final manual visual review has not run and no current candidate exists. This matrix is therefore a future local exact-checkout qualification contract, not a claim that current-HEAD exact-tag browser, RSS, or rollback gates passed; GitHub Node CI is not a substitute or a release requirement.
