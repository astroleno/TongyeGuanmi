...
Status: candidate-v5 artifact/static-shell verification, exact HTTP no-JS smoke, and the applicable release no-JS cases passed, but v5 remains unqualified because five unrelated media-oracle cases failed the release matrix. The exact-v6 no-JS qualification runs last with the release matrix and remains an external post-freeze record.
...
Desktop/mobile Chromium/WebKit no-JS cases from candidate-v2 remain baseline history only. Candidate-v5's clean exact-tag HTTP shell smoke and no-JS browser cases passed, but its later release E2E failures prevent carry-forward. The clean exact-v6 identity smoke and final exact-tag no-JS rerun are recorded externally after freeze; no earlier qualified R5 tag contains the complete lifecycle/release/browser closure.
