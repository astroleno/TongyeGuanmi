...
本 roadmap 最初作为替代性总体规划创建；R0-R4 已落地，R5 candidate-v2 为 immutable `NEEDS WORK`。Candidate-v3 exact RSS 通过，但 clean-tree finalization 失败，因此保持 immutable unqualified；runner/workflow 已在 `e71b970` 修复。当前进入 candidate-v4 exact tag → identity-bound RSS/finalization → rollback → E2E-last 资格序列。旧站基线固定为 `react-refactor-legacy-static-baseline`；state-machine / scene-runtime 系列分支继续只作历史参考。
...
**状态**：candidate-v2 的身份冻结有效但为 `NEEDS WORK`；candidate-v3 为 immutable unqualified。`docs/plans/2026-07-13-003-fix-r5-candidate-v3-lifecycle-gates-plan.md` 的运行期实现已完成，release runner 的末端缺口也已关闭；只在 candidate-v4 exact identity-bound RSS/finalization、rollback 和最终 E2E 全绿后，才交给 HITL。尚未合并/部署 `main`，也未建立 `react-refactor-r5-cutover`。
...
```txt
R-1 → R0 → R1 → R2 → R3 → R4 closeout → candidate-v2 (`NEEDS WORK`) → candidate-v3 (unqualified) → candidate-v4 qualification → HITL cutover → R6
                  ↑
