...
本 roadmap 最初作为替代性总体规划创建；R0-R4 已落地，R5 正在完成 HITL regression closure 的 candidate-v2 最终复验。旧站基线固定为 `react-refactor-legacy-static-baseline`；state-machine / scene-runtime 系列分支继续只作历史参考。
...
**状态**：旧 parity-repair candidate 经 HITL 拒绝；九项 regression closure 的实现已完成，正在从最终精确源码重跑 root/browser/performance/memory/rollback gate。全部通过后只创建 `react-refactor-r5-parity-repair-candidate-v2` 并停止等待 HITL；尚未合并/部署 `main`，也未建立 `react-refactor-r5-cutover`。
