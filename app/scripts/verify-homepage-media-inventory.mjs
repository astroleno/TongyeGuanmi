...
assert(inventorySources.length === 56, `expected 56 homepage source entries, found ${inventorySources.length}`);
assert(frozenHomepageMedia.length === 56, `expected 56 frozen homepage media entries, found ${frozenHomepageMedia.length}`);
...
assert(portraitOnlyImageSources.length === 4, `expected 4 portrait-only WebP sources, found ${portraitOnlyImageSources.length}`);
...
assert(packedAlphaVideoSources.length === 6, `expected 6 packed alpha H.264 sources, found ${packedAlphaVideoSources.length}`);
...
assert(emittedMedia.length === 56, `expected exactly 56 emitted homepage media files, found ${emittedMedia.length}`);
...
assert(emittedWebp.length === 34, `expected exactly 34 emitted WebP files, found ${emittedWebp.length}`);
...
const hevcInventory = inventory.filter(({ source }) => animationHevcAlphaSources.includes(source));
const packedAlphaVideoInventory = inventory.filter(({ category }) => category === 'portrait-packed-alpha');
...
const report = {
  ...
  actual: {
    ...
    iosStaticPathBytes,
    portraitPackedAlphaStaticPathBytes,
    largestHomepageMediaBytes,
    inventoryFileCount: inventory.length,
    animationWebmCount: emittedWebm.length,
    animationHevcAlphaCount: hevcInventory.length,
    portraitPackedAlphaCount: packedAlphaVideoInventory.length,
    mp4Count: emittedMp4.length,
...
process.stdout.write(`${JSON.stringify({
  ...
  animationHevcAlpha: hevcInventory.length,
  portraitPackedAlpha: packedAlphaVideoInventory.length,
  mp4: emittedMp4.length,
