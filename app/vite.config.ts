...
for (const propertyName of phoneCrossChunkContractPolicy.dynamicPropertyNames) {
  if (!phoneCrossChunkReservedProperties.has(propertyName)) {
    throw new Error(
      `Phone cross-chunk dynamic property is missing from mangle reserve (${propertyName})`
    );
  }
}
    ...
    terserOptions: {
      ...
      mangle: {
        properties: {
          ...
          // The timeline driver and transition controllers retain their state
          // behind TypeScript-private members. These names are never returned
          // from an adapter, read through a string key, or carried across a
          // lazy boundary; only their public, contract-reserved methods cross
          // chunks. Mangling them recovers release headroom without changing
          // any raw-frame, token, DOM, or adapter contract.
          regex: /^(?:entrySettled|startDelayMs|revealMs|holdMs|renderedProgress|figureDrawn|flockDrawn|armedLeg|preparedLeg|endScale|renderActive|centerX|centerY|liveMotionPhase|completeMedia|entryReady|animate|presentationRevision|firstFrameProof|readiness|clearEndpoints|presentationProofToken|presentationFrameToken|startY|requestReducedTargetLayout|system|_snapshot|states|layerWindow|machine|recovery|segmentPlayer|settlingTarget|buildTimeoutMs|settled|activeCursor|settleRun|mailbox|_processingStatus|_parent|actions|defaults|output|pendingDirection|visibilityByScene|initial|template|actorEpoch|staged|invoke|observers|pendingSegment|handles|rawFrame|presentedLeg|staticTarget|releaseExtra|runForVisual|direct|samples|contour|spec|boundaryRank|ownership|fieldKind|targetImage|depthSrc|occlusion|isActive|revealSurface|targetTime|onAbort|desired|leaseOwners|baseOwners|motionLeases|surfaceHost|resizeObserver|fieldSpec|fieldContour|inkRenderer|renderSource|prepareEndpoints|easing|sampleAt|copyCueActive|copyCueActivations|memoizedState|flags|stateNode|sibling|alternate|lanes|updateQueue|memoizedProps|pendingProps|subtreeFlags|childLanes|authorityId|diagnostics|lastRollback|completedEpoch|completedEpochUntil|sampleRevision|projection|operation|inputEpoch|anchor|alignment|geometryRevision|targetY|commandId|correctionCount|confirmedY|visualViewportOffsetTop|fallbackScene|cinematic|corridor|status|revision|scroll|input|sessionId|generation|legIndex|direction|phase|progress|actualY|run|scene|from|to|segment|runSource|runTarget|trigger|anchorY|commitState|checkpoint|edge|stageOwner|stageScene|sourceSurface|receiverSurface|effects|inputDisposition|boundaryKnown|crossedBoundary|scrollCorridors|hasPresentedSurface|sampleNow|scenes|sample|boundary|landing|viewportWidth|viewportHeight|reason|kind|source|directScene|disposed|resolveIntent|syncDiagnostics|registerRunCapability|registerSurface|registerScrollCorridor|registerTransitionEndpoints|clearTransitionEndpoints|reapplyCurrent|preflight|rootForScene|canStart|startAtLeg|reportPresentedFrame|reportProgress|reportEndpoints|reportEndpointCommit|reportTargetPresented|reportEndpointRelease|provideRelease|reportAnimationComplete|reportFailure|releaseGeometry|releaseResources|onNativeScrollCorrection|scrollState|wheelQuietMs|momentumWindowMs|requestFrame|cancelFrame|coverageRoot|presented|attach|coverageSurface|landingResolver|semanticScene|navigationScene|initialScene|scheduleFrame|ownerId|capability|dependencies|resume|valid|endpoints|checkpointTrace|retention|abortController|visualScenes|capabilities|reducedMotion|timeoutMs|directLegIndex|runId|preparation|boundaries|documentSurface|presentation|evidence|latestObservedAt|observedAt|subject|frameKinds|connected|visible|coverage|content|readSurfacePresentation|prepareDirectEntry|layerPlan|placement|zIndex|role|publishing|controller|timeout|key|requested|reported|paintFrame|proofFrame|progressValue|playbackDirection|renderedSourceDirection|latest|latestInput|queuedSeek|inFlightSeek|primingSeek|waiters|nativeFallback|nativeStarted|nativeAttempt|pendingNative|reportedTimelineReady|lastDepthFrameRendered|stagedPlaybackFor|fieldRoots|animateRange|phaseRanges|rotationProgress|transformRoot|fixedFrom|fixedTo|mediaDirection|sourceElevation|depthMask|inkCanvas|inkRuntime|ownsMedia|nativePlayback|toElement|fromElement|frameReadyGeneration|readyTime|frameCallbackId|presentingFrame|presentedSeekFrame|seekedFrameFallbackTimer|seekedFrameFallbackFrame|onSeeked|onLoadedMetadata|onMediaAbort|desiredFrame|driveNative|startNativePlayback|stopNativePlayback|scheduleSeek|beginExactTargetSeek|completePrimingSeek|flushQueuedSeek|completeInFlightSeek|presentFrame|armSeekedFrameFallback|cancelSeekedFrameFallback|markFrameReady|cancelFrameCallback|resolveWaiter|rejectWaiter|rejectWaitersForFrame|failFrame|cancelPresentationWhenUnused|writeDiagnostics|configureElement|failAllWaiters|firstFramePresented|lastProgress|playConfirmed|phaseController|endpointOwnership|preparedFromRoot|preparedToRoot|endpointsPrepared|rendererError|rendererFailure|liveElevation|liveElevationElement|renderedSourceRoot|renderedSourceProgress|revealSurfaces|concealSurfaces|sourceRenderContext|replaceManagedSurfaces|ensureEndpointsPrepared|ringTextureIndex|dynamicHighlightCanvas|flowerContext|petalContext|flowerCanvas|petalCanvas|ringCanvases|terminalRingCanvases|lastFlowerTextureSize|lastFlowerPhase|motionElapsedSeconds|motionStartedAt|staticFrameRequested|ringStructuralKey|framePending|animateMotion|lastRenderedAt|frameRevision|resolveReady|readyPromise|cameraHighlightCanvas|noiseMaskCanvas|sourceData|renderNoiseOverlay|buildHighlightSource|buildDynamicHighlight|renderCameraOverlays|drawOutputGlow|drawCameraCanvasTo|drawCoveredCanvas|drawCameraCanvas|animatedNoiseField|resizeOutput|resetContext|prepareSource|buildSourceTextures|renderSourceFlowerTexture|drawDecorLayers|drawSourceFlower|drawRingTexture|refreshRingTextures|buildNextRingTexture|drawOuterPetalKaleidoscope|buildRingCache|drawPetalField|motionElapsed|renderFrame|requestRender|getObjectMetrics|loadImage|textureSize|dpr|rafId|layers|frameRevision|destroyed|drawSize|rotationBase|startCanvas|endCanvas|highlightCanvas|sourceCanvas|cloudYVh|figureShiftYVh|alphaEndProgress|driveReverseFrame|disposeReverseDriver|videoOpacity|videoScale|backdropOpacity|backdropScale|preparationKey|onPresented|readyMediaTime|presentedSeekMediaTime|exactFrameRetryTimer|assertRendererReady|attrsElement|animateTo|armDepthMask|armExactFrameRetry|cancelExactFrameRetry|activate|abortError|markStaticFallback|endpointSeekStage|activePresentationToken|activeFrameToken|exactForwardFrameRequired|exactForwardFrameListener|presentationGeneration|activationReady|resolveActivationReady|queuedPresentationToken|settleActivation|clearExactForwardFrame|rebindPresentationToken|setupActive|waitForRetainedContext|initializeCanvas|retireCanvas|clearPresentation|failEndpoint|rejectSupersededPreparations|timelineVideoFrameMediaTime|resolveAllWaitersAsStale|disposeDriver)$/,
          builtins: true,
