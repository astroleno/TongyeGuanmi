...
#elif defined(FR)
float rc(vec2 d,float as){vec2 q=d/max(length(d),0.000001);if(length(d)<.000001)q=vec2(1.0,0.0);float a=fract(atan(q.y,q.x)/6.2831853);vec3 hs=texture2D(M,vec2(a,0.5)).rgb*2.0-1.0;float n=dot(hs,vec3(0.50,0.31,0.19));float e=sin(clamp(H,0.0,1.0)*3.14159265);return max(1.0+n*${RADIAL_INK_CONTOUR_AMPLITUDE.toFixed(6)}*e,0.0001);}float rr(vec2 u,float as){vec2 d=(u-O)*vec2(as,1.0);return length(d)/max(Z*rc(d,as),0.0001);}
#if FT
