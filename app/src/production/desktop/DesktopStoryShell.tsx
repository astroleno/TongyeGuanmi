...
import {
  loadDesktopSceneModule as loadSceneModule,
  loadDesktopTransitionModule as loadTransitionModule,
  loadedDesktopModules as loadedProductionModules
} from './module-loaders';
import {
...
export type DesktopStoryShellSnapshot = {
  phase: string;
  current: SceneId;
...
export type DesktopStoryShellApi = {
  navigate(scene: SceneId, source?: DirectorSeekSource): Promise<void>;
  snapshot(): DesktopStoryShellSnapshot;
};

export type DesktopStoryShellProps = Readonly<{
...
export function DesktopStoryShell({ spikeRoute }: DesktopStoryShellProps = {}) {
  ...
  const readSnapshot = useCallback((): DesktopStoryShellSnapshot => {
  ...
  useEffect(() => {
    const api: DesktopStoryShellApi = {
...
declare global {
  interface Window {
    ...
    __storyApp?: DesktopStoryShellApi;
  }
}
