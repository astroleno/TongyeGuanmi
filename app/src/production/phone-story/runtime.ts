...
const PHONE_HANDOFF_BLOCK_REASONS = ['mount', 'surface', 'coverage', 'generation', 'prewarm'] as const;
export type PhoneNativeHandoffBlockReason = (typeof PHONE_HANDOFF_BLOCK_REASONS)[number];
...
export type PhoneStoryRuntime = Readonly<{
  ...
  nativeHandoff(direction: 'forward' | 'reverse'): PhoneNativeHandoffState;
}>;
...
export function createPhoneStoryRuntime(config: PhoneStoryRuntimeConfig): PhoneStoryRuntime {
  ...
  let playback: PlaybackLease | null = null, planeFrame: PhoneRuntimeTimerHandle | null = null, playbackController: AbortController | null = null, playbackRequest: PhoneMediaFrameRequest | null = null;
  ...
  const inspectNativeHandoffLease = (direction: 'forward' | 'reverse'): readonly [LeafLease | null, -1 | 0 | 1 | 2 | 3 | 4 | null] | null => {
    ...
    const lease = candidates.find((candidate) => candidate.mount.isAttached());
    if (!lease) return [null, candidates.length ? 3
      : [...reportStates].some((candidate) => candidate.p === descriptor[0]) ? 1 : 0];
    return [lease, activationCoverageComplete([lease], required) ? null : 2];
  };
  ...
  const cancelDeadlines = (predicate: (lease: DeadlineLease) => boolean): void => { runPhoneCleanupSteps('Phone deadline cancellation failed', [...deadlines.values()].filter(predicate).map((lease) => () => cancelDeadline(lease.key))); };
  ...
  const cancelPlayback = (): void => { if (playback) environment.cancelFrame(playback.h); playbackController?.abort(); playback = null; playbackRequest = null; };
  ...
  const syncPlayback = (activeConnection: number): void => {
    ...
    const { attempt, stageIndex, progress: from } = active;
    const segment = phoneManifest.segments.find(({ id }) => id === attempt.segmentId);
    if (!segment || !attempt.direction) return cancelPlayback();
    const initialFrame = phoneSegmentChoreographyFrame(attempt.segmentId!, from, attempt.direction, stageIndex);
    const frameLock = initialFrame.mediaClockMode === 'frame-lock';
    if (playback && sameAttempt(playback.a, attempt) && playback.s === stageIndex) return;
    cancelPlayback();
    const policy = segment.timing.policy, staged = policy.kind === 'stagedSnap' ? policy : null, reverse = attempt.direction === 'reverse', playIndex = staged && reverse ? staged.playMs.length - 1 - stageIndex : stageIndex, durationMs = Math.max(1, staged ? staged.playMs[playIndex] ?? 1 : segment.timing.virtualDuration);
    const boundaryIndex = staged && reverse ? staged.stops.length - 1 - stageIndex : stageIndex, to = staged && stageIndex < staged.playMs.length - 1 ? staged.stops[boundaryIndex] ?? from : reverse ? 0 : 1, startedAt = environment.activeNow(), controller = new AbortController();
    const failFrame = (owner: 'source' | 'target' | null, code: string, message: string): void => { cancelPlayback(); const slot = snapshot.status === 'transaction' && snapshot.transaction.requiredPrepared.find(({ leg }) => leg === owner); if (slot) enqueueFor({ type: 'failure-reported', slot, failure: { code, message, recoverable: true } }, activeConnection); };
    const tick = (): void => {
      if (!ownsConnection(activeConnection)) return cancelPlayback();
      const handle = environment.requestFrame(() => {
        const lease = playback;
        if (!ownsConnection(activeConnection) || !lease || lease.h !== handle) return;
        const ratio = Math.min(1, Math.max(0, (environment.activeNow() - lease.t) / lease.d));
        const desiredProgress = lease.f + (lease.o - lease.f) * ratio;
        if (!frameLock) {
          enqueueFor({ type: 'transition-progressed', attempt: lease.a, progress: desiredProgress }, activeConnection);
          if (ratio < 1 && ownsConnection(activeConnection) && playback?.h === handle) tick();
          else if (playback?.h === handle) { playback = null; enqueueFor({ type: 'transition-completed', attempt: lease.a }, activeConnection); }
          return;
        }
        ...
        if (mediaOwner === 'none' || !token || !presentFrame) {
          failFrame(mediaOwner === 'none' ? null : mediaOwner, 'media-frame-presenter-unavailable', 'owner unavailable');
          return;
        }
        const request: PhoneMediaFrameRequest = { frameToken: token, transactionId: lease.a.transactionId, direction: reverse ? -1 : 1, sequence: ++sequence, desiredProgress, signal: playbackController!.signal }; const { sequence: seq, desiredProgress: wanted } = request;
        const ownsFrameRequest = (): boolean => ownsConnection(activeConnection) && playbackRequest === request && snapshot.status === 'transaction' && sameAttempt(snapshot.transaction.attempt, lease.a) && snapshot.transaction.stageIndex === lease.s;
        playbackRequest = request;
        void presentFrame(request).then((receipt) => {
          if (!ownsFrameRequest()) return;
          if (receipt.status !== 'presented' || receipt.frameToken !== token || receipt.sequence !== seq || receipt.desiredProgress !== wanted || !Number.isFinite(receipt.presentedProgress)) { failFrame(mediaOwner, 'media-frame-present-failed', 'receipt invalid'); return; }
          playbackRequest = null;
          enqueueFor({ type: 'transition-progressed', attempt: lease.a, progress: receipt.presentedProgress, presentedSequence: receipt.sequence }, activeConnection);
          if (environment.activeNow() >= lease.t + lease.d) enqueueFor({ type: 'transition-completed', attempt: lease.a }, activeConnection); else tick();
        }, (error: unknown) => { if (!ownsFrameRequest()) return; failFrame(mediaOwner, 'media-frame-present-failed', error instanceof Error ? error.message : String(error)); });
      });
      playback = { h: handle, a: attempt, s: stageIndex, t: startedAt, d: durationMs, f: from, o: to };
  ...
  function createReportPort(state: ReportState): PhoneLeafReportPort {
    return Object.freeze({
      ...
      reportPrepared: (surfaceId, report) => {
        ...
        if (state.p) { if (!report.ready || report.kind !== 'video-decoded' || !lease || !lease.mount.surfaceIds.includes(surfaceId) || lease.mount.surfaceIds.some((id) => id.includes('canvas'))) return; state.r = true; publishPrewarm(); return; }
      ...
      reportFrame: (surfaceId, report) => {
        ...
        if (state.p) { state.r = true; publishPrewarm(); return; }
        acceptPreparedProof(state, lease, { surfaceId, report });
      },
      reportProgress: () => undefined,
      reportComplete: () => undefined,
      reportFailure: (failure: PhoneFailure) => { if (state.p) { rejectedLoads.add(`scene:${state.p}`); publishPrewarm(); return; }
  ...
  function invokeActivation(
    ...
    if (candidates.length > 0 && candidates.every(({ reports }) => reports.r)) { for (const owner of candidates) { delete owner.reports.p; delete owner.reports.r; owner.mount.commands.setMediaPhase?.({ phase: 'primed', runToken, direction, stageIndex }); } enqueueFor({ type: 'activation-settled', invoked: true, attempt }, activeConnection); return; }
  ...
  const activationCoverageComplete = (candidates: readonly LeafLease[], surfaceIds: readonly PhoneSurfaceId[]): boolean => { const covered = new Set(candidates.flatMap((lease) => phoneActivationSurfaceIds(lease.mount, surfaceIds))); return surfaceIds.every((id) => covered.has(id)); };
  ...
  const driveCommandHandles = (
    ...
    if (transaction.phase === 'presenting-target' && previous.transaction.phase !== 'presenting-target') {
      for (const lease of leases) {
        ...
        if (settled?.prewarmReusable === false) { delete lease.reports.p; delete lease.reports.r; }
      }
    }
  };
      ...
      if (next.stableCommit !== previous.stableCommit && ports.prewarmDependencies) {
        ...
        void ports.prewarmDependencies(effect, controller.signal)
          ...
          .catch((error: unknown) => {
            ...
            for (const scene of phoneNativePrewarmScenes(next.stableCommit.sceneId)) rejectedLoads.add(`scene:${scene}`);
            publishPrewarm();
            void chunkRecovery.reportRejectedChunk({
              authorityId: effect.attempt.authorityId,
              transactionId: effect.attempt.transactionId,
  ...
  const handleHost = (event: PhoneRuntimeHostEvent, expectedConnection: number): void => {
    ...
    if (event.type === 'input') {
      ...
      if (direction) {
        const descriptor = snapshot.status === 'stable' && snapshot.stableCommit ? phoneNativeHandoffDescriptor(snapshot.stableCommit.sceneId, direction) : null;
        const token = descriptor ? nativeHandoff(direction)[0] : null;
        if (descriptor && (!token || event.handoffToken !== token)) return;
        const epoch = ++physicalEpoch, active = snapshot.status === 'transaction' ? snapshot.transaction : null;
        enqueueFor(active?.phase === 'awaiting-leg-intent' && active.attempt.direction === direction
          ...
          : { type: 'segment-requested', direction, physicalEpoch: epoch, reducedMotion: environment.readReducedMotion() }, expectedConnection);
        if (descriptor && snapshot.status === 'transaction') for (const lease of [...leaves.values()]) { if (lease.reports.p && lease.reports.p !== descriptor[0]) retireLease(lease, 'closure-retired'); }
      }
  ...
  return Object.freeze({
    ...
    createPrewarmLeafReportPort: (sceneId: PhoneSceneId) => { for (const stale of [...reportStates]) if (stale.p === sceneId && !mountedLease(stale)) closeReports(stale, false); const scene = phoneSceneById(sceneId); const state: ReportState = { p: sceneId, valid: true, binding: { attempt: { authorityId: snapshot.authorityId, transactionId: `prewarm:${sceneId}`, transactionGeneration: 0, mode: 'recovery', sceneId, segmentId: null, direction: 'forward' }, stageIndex: 0, leg: 'target', allowedReports: [], allowedSurfaceIds: scene.surfaces, planeRevision: null } }; reportStates.add(state); return createReportPort(state); },
    nativeHandoff
  });
}
