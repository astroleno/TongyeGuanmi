...
export type PortraitInkTransition = Readonly<{
  render(progress: number): void;
  dispose(): void;
}>;

type PortraitInkTransitionOptions = Readonly<{
 ...
 */
export function createPortraitInkTransition(
  options: PortraitInkTransitionOptions
): PortraitInkTransition {
  const host = options.host;
  const surface = mountTransitionInkCanvas(host, options.id, {
    renderer: 'field',
    grade: options.grade ?? 'dark',
    generation: `portrait-spike:${options.id}`,
    className: 'portrait-scroll-spike__ink'
  ...
  surface.dataset.portraitInkRenderer = renderer ? 'active' : 'unavailable';
  if (renderer) {
  ...
  return {
    render(rawProgress) {
      ...
      surface.dataset.portraitInkProgress = progress.toFixed(4);
    ...
    dispose() {
      ...
      delete surface.dataset.portraitInkRenderer;
      delete surface.dataset.portraitInkProgress;
    }
  };
}
