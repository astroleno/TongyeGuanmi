...
import {
  PORTRAIT_SPIKE_BELIEF_COPY,
  PORTRAIT_SPIKE_HOME_COPY,
  PORTRAIT_SPIKE_METHOD_COPY
} from './copy';
import {
  attachPortraitDeviceParallax,
  createPortraitFigurePlayback,
  type PortraitFigurePlayback
} from './portrait-motion';
import {
  createPortraitInkTransition,
  type PortraitInkTransition
} from './portrait-ink';
import {
...
import './PortraitScrollSpike.css';
...
const HERO_FIGURE_PACKED_ALPHA_VIDEO = new URL('../../../../assets/figure1-rgb-alpha.mp4', import.meta.url).href;
const AOD_FIGURE_PACKED_ALPHA_VIDEO = new URL('../../../../assets/aod-figure-motion-rgb-alpha.mp4', import.meta.url).href;
const AOD_FIGURE_PACKED_ALPHA_REVERSE_VIDEO = new URL('../../../../assets/aod-figure-motion-rgb-alpha-reverse.mp4', import.meta.url).href;
...
const HERO_COPY = PORTRAIT_SPIKE_HOME_COPY;
...
const HERO_SUBTITLE_LINES = HERO_SUBTITLE_BREAK > 0
  ...
  : [HERO_SUBTITLE];
const PATTERN_COPY = PORTRAIT_SPIKE_BELIEF_COPY.slice(0, 3);
const STAR_MAP_COPY = PORTRAIT_SPIKE_BELIEF_COPY[3]!;
const METHOD_TOP_COPY = PORTRAIT_SPIKE_METHOD_COPY.slice(0, 8);
const METHOD_STEPS_COPY = PORTRAIT_SPIKE_METHOD_COPY.slice(8, 23);
...
export function PortraitScrollSpike() {
  ...
  const [loaderHidden, setLoaderHidden] = useState(portraitLoaderCompletedInDocument);
  const [heroTitleActive, setHeroTitleActive] = useState(
    () => portraitLoaderCompletedInDocument() || !motionEnabled
  );
  ...
  useEffect(() => attachPortraitLoaderVisibilityLifecycle(), []);
  ...
  useGSAP(() => {
    ...
    let active = true;
    let figurePlayback: PortraitFigurePlayback | undefined;
    let figureCompositor: PackedAlphaVideoCompositor | undefined;
    let aodCompositor: PackedAlphaVideoCompositor | undefined;
    let aodAutoplay: PortraitAodAutoplay | undefined;
    let heroInk: PortraitInkTransition | undefined;
    let patternInk: PortraitInkTransition | undefined;
    let starInk: PortraitInkTransition | undefined;
    ...
    const aodScrollSnap = createPortraitScrollSnapLock({
      root,
    ...
    const renderAodFrame = (rawProgress: number) => {
      ...
      const methodProgress = portraitAodMethodProgress(progress);
    ...
    const completeAodRun = (direction: PortraitAodPlaybackDirection) => {
    ...
    if (motionEnabled) {
      ...
      figurePlayback = createPortraitFigurePlayback(
        heroFigureVideo,
      ...
      aodAutoplay = createPortraitAodAutoplay(aodFigureVideo, {
      ...
      aodAutoplay.reset();
      const deviceParallax = attachPortraitDeviceParallax({
        root,
        targets: [
      ...
      heroInk = createPortraitInkTransition({
        host: stage,
      ...
      patternInk = createPortraitInkTransition({
        host: stage,
      ...
      starInk = createPortraitInkTransition({
        host: stage,
