...
function segment(): SpineSegmentNode {
  const found = storyManifest.nodes.find((node): node is SpineSegmentNode => node.kind === 'segment' && node.id === 'method-top-method-bottom');
  if (!found) {
    throw new Error('method-top-method-bottom segment missing');
  }
...
function context(prefersReducedMotion = false): TransitionContext {
  return {
    segment: segment(),
    from: layer('method-top', 'current'),
    to: layer('method-bottom', 'next'),
    stage: {
      getLayer: () => undefined,
...
describe('method-top-method-bottom transition', () => {
  ...
  it('is idempotent across 0 to 1 to 0 to 1 progress', async () => {
    const timeline = await createMethodTopMethodBottomTransition().buildTimeline(context());
  ...
  it('collapses duration in reduced motion', async () => {
    const timeline = await createMethodTopMethodBottomTransition().buildTimeline(context(true));
