...
float organicNeuroTaste(vec2 uv, float t, float p){
  ...
  for(int j = 0; j < 7; j++){
    uv = rot(0.96) * uv;
    sineAcc = rot(1.04) * sineAcc;
    vec2 layer = uv * scale + float(j) + sineAcc - t;
    sineAcc += sin(layer) + 1.8 * p;
    res += (0.5 + 0.5 * cos(layer)) / scale;
    scale *= 1.2;
  }

  float n = res.x + res.y;
  n = 1.18 * pow(max(n, 0.0), 3.0);
  n += 0.38 * pow(max(n, 0.0), 8.0);
  return max(0.0, n - 0.44);
}
...
vec4 sceneA(float idx){
  ...
  if(idx < 2.5) return vec4(3.35, 0.28, 0.18, 13.0);  // S03 四个现场展开
...
vec4 sceneB(float idx){
  ...
  if(idx < 2.5) return vec4(0.72, 0.16, 0.00, 0.10);
...
void main(){
  ...
  float organic = organicNeuroTaste(q * (0.46 + A.x * 0.025), u_time * 0.62 + u_scroll * 2.0, A.y);
  ...
  float vignette = smoothstep(1.26, 0.10, length(p * vec2(0.92, 0.88)));
  float grain = (noise(uv * u_resolution.xy * 0.38 + u_time * 0.18) - 0.5) * 0.045;
  ...
  bg += 0.032 * vec3(fbm(p * 2.1 + 7.0), fbm(p * 2.0 + 17.0), fbm(p * 2.2 + 29.0));
  bg += 0.030 * moss * smoothstep(0.9, 0.1, length(p - vec2(-0.45, 0.18)));
  ...
  float fiberMass = neuro * 0.92 + organic * 0.52 + branch * 0.50 + orbit * 0.34 + pipe * 0.20;
  float fiberCore = smoothstep(0.16, 0.72, fiberMass);
  float fiberGlow = pow(max(fiberMass, 0.0), 1.38);
  vec2 diagonal = rot(-0.48) * q;
  float warmVein = exp(-abs(diagonal.x + 0.10 * sin(diagonal.y * 4.2 + u_time * 0.16)) * 5.4);
  warmVein *= smoothstep(1.05, 0.12, abs(diagonal.y));

  color += warmGold * fieldMapLift * (0.020 + 0.034 * fbm(q * 1.4 + 2.0));
  color += neuro * mix(moss, warmGold, 0.78) * (0.64 + fieldMapLift * 0.24);
  color += organic * mix(moss, warmGold, 0.66) * (0.38 + fieldMapLift * 0.12 - conversionCalm * 0.08);
  color += orbit * warmGold * 0.58;
  color += branch * mix(ivory, warmGold, 0.52) * (0.62 + fieldMapLift * 0.24);
  color += grid * mix(silver, moss, 0.18) * (0.56 + fieldMapLift * 0.18);
  color += pipe * mix(warmGold, ivory, 0.25) * 0.52;
  color += converge * mix(warmGold, ivory, 0.38) * 0.66;
  color += particle * ivory * 0.42;
  color += warmGold * warmVein * (0.018 + fieldMapLift * 0.014 + openFieldLift * 0.008);
  color += mix(warmGold, ivory, 0.34) * fiberGlow * (0.075 + fieldMapLift * 0.070 + openFieldLift * 0.035);
  color += ivory * fiberCore * (0.020 + fieldMapLift * 0.016);
  ...
  color *= tint;
  color *= mix(0.34, 0.94, vignette);
  color += grain;
  color *= 0.88 + 0.10 * smoothstep(0.02, 0.18, u_scroll);
  color *= u_intensity;
